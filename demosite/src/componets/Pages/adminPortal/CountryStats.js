...
class CountryStats extends Component {
  ...
  async componentWillMount() {
    await fetch(`/users/getstateStats`)
      .then((respone) => respone.json())
      .then((Result) => {
        var y = 1;
        for (var i in Result.data) {
          if (y == 1) {
            this.setState({ city1: Result.data[i].state });
            this.setState({ countries: Result.data[i].country });
            this.setState({ count1: Result.data[i].total });
          } else if (y == 2) {
            this.setState({ city2: Result.data[i].state });
            this.setState({ count2: Result.data[i].total });
          } else if (y == 3) {
            this.setState({ city3: Result.data[i].state });
            this.setState({ count3: Result.data[i].total });
          } else if (y == 4) {
            this.setState({ city4: Result.data[i].state });
            this.setState({ count4: Result.data[i].total });
          } else if (y == 5) {
            this.setState({ city5: Result.data[i].state });
        ...
        <Pie
          ...
          options={{
            title: {
              display: true,
              text:
                `Number Of Cases in each Province of ` + this.state.countries,
              fontSize: 20,
            },
            legend: {
              display: true,
